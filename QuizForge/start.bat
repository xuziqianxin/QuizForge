...
:: 检查 orjson（可选，高性能 JSON 加载）
python -c "import orjson" >nul 2>nul
if %errorlevel% neq 0 (
    echo [提示] 可选依赖 orjson 未安装（JSON 加载将使用标准库，速度稍慢）。
    echo 是否安装 orjson？可大幅提升大型题库的搜索速度。
    choice /c YN /n /m "安装 orjson？[Y/N] (默认 N): "
    if !errorlevel! equ 1 (
        set NEED_INSTALL=1
        set INSTALL_ORJSON=1
    )
)
...
if !NEED_INSTALL! equ 1 (
    ...
    :: 安装 orjson（如果用户选择安装）
    if defined INSTALL_ORJSON (
        echo 正在安装 orjson...
        python -m pip install orjson
        if !errorlevel! neq 0 (
            echo [警告] orjson 安装失败，程序仍可正常运行（将使用标准库 json）。
        )
    )
